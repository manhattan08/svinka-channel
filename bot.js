...
bot.on('chat_member', async (ctx) => {
    try {
        ...
        if (update.chat.id.toString() !== process.env.PRIVATE_CHANNEL_ID) {
            return;
        }
        ...
        if (newStatus === 'member' && oldStatus !== 'member') {
            ...
            if (subscription && subscription.invite_link) {
                try {
                    ...
                    console.log(`Отозвана ссылка для пользователя ${userId} после присоединения`);
                } catch (revokeError) {
                    console.error(`Ошибка при отзыве ссылки для пользователя ${userId}:`, revokeError);
                }
            }
        }
        ...
        if ((oldStatus === 'member' || oldStatus === 'administrator' || oldStatus === 'creator') &&
            ...
            if (subscription && subscription.invite_link) {
                try {
                    ...
                    console.log(`Отозвана ссылка для пользователя ${userId} после выхода из канала`);
                } catch (revokeError) {
                    console.error(`Ошибка при отзыве ссылки для пользователя ${userId} после выхода:`, revokeError);
                }
            }
        }
    } catch (error) {
